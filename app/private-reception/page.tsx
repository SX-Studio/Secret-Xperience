...
export default function PrivateReceptionPage() {
  ...
  // Filter toast for arrivals from homepage pill dropdowns
  ...
  useEffect(() => {
    ...
    let label = ''
    if (g === 'female')      label = 'Vrouwen'
    else if (g === 'male')   label = 'Mannen'
    else if (g === 'trans')  label = 'Transgenders'
    else if (g === 'couple') label = 'Koppels'
    else if (g === 'gay')    label = "Homo's"
    else if (c === 'companionship') label = 'Companionship'
    else if (c === 'massage')       label = 'Massage'
    else if (c === 'domination')    label = 'Dominantie'
    else if (c === 'experiences')   label = 'Experiences'
    if (!label) return
          ...
          <main style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ marginTop: '4rem', background: 'var(--bg1)', border: '0.5px solid var(--b2)', borderRadius: 'var(--rl)', padding: '2.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '2rem', flexWrap: 'wrap' }}>
              <div>
                ...
                <p style={{ color: 'var(--t2)', fontSize: '14px', lineHeight: 1.65, maxWidth: '420px' }}>Receive clients at your location. Free advertisement, discreet, no middlemen.</p>
              </div>
