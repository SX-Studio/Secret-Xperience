...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticRoutes = ['/', '/events', '/advertise', '/search', '/discover', '/live', '/nightlife', '/venues', '/swingers', '/fetish', '/jobs', '/terms', '/privacy', '/login', '/partners', '/creator-platforms', '/regulations', '/medical', '/refer', '/creators', '/shop', '/how-it-works', '/why-secretxperience'].map(path => ({ url: BASE + path, lastModified: new Date(), changeFrequency: path === '/' ? 'daily' as const : 'weekly' as const, priority: path === '/' ? 1 : path === '/why-secretxperience' ? 0.95 : ['/search', '/discover', '/nightlife', '/events', '/advertise', '/regulations', '/medical', '/swingers', '/fetish', '/jobs'].includes(path) ? 0.8 : 0.7 }))
