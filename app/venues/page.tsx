    ...
    <div style={{ background: '#080612', minHeight: '100vh', color: '#ece8e1' }}>
      ...
      <section style={{ padding: '4.5rem 1.5rem 3rem', textAlign: 'center', position: 'relative', overflow: 'hidden', borderBottom: '0.5px solid rgba(255,255,255,0.06)' }}>
        ...
        <div style={{ position: 'relative', maxWidth: '720px', margin: '0 auto' }}>
          <div style={{ font: '700 11px/1 Poppins, sans-serif', letterSpacing: '0.3em', color: '#c5a05a', textTransform: 'uppercase', marginBottom: '1.25rem' }}>Venues · Belgium · Netherlands · Germany · France</div>
          ...
          <p style={{ fontSize: '15.5px', color: 'rgba(236,232,225,0.6)', lineHeight: 1.7, maxWidth: '580px', margin: '0 auto 1.5rem' }}>
            A curated directory of {venues.length} adult venues across Belgium, the Netherlands, Germany &amp; France — gentlemen&rsquo;s clubs, erotic massage salons, saunas, brothels, domina studios and swingers clubs. Each links directly to its own website.
          </p>
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
            {[['belgium','🇧🇪 Belgium'],['netherlands','🇳🇱 Netherlands'],['germany','🇩🇪 Germany'],['france','🇫🇷 France']].map(([id, label]) => (
              <a key={id} href={`#${id}`} style={{ padding: '8px 18px', borderRadius: 20, border: '0.5px solid rgba(197,160,90,0.4)', color: '#c5a05a', textDecoration: 'none', font: '600 13px/1 Poppins, sans-serif' }}>{label}</a>
            ))}
          </div>
        </div>
      </section>
