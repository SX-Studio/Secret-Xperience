...
// Verotel FlexPay signature (proven algorithm, matching the SX integration):
//   text = SIGNATURE_KEY
//   for each param sorted case-insensitively by name (excluding "signature"
//   and empty values): text += ":" + name + "=" + value
...
// success/decline URLs are configured in the Verotel panel (FlexPay options),
// NOT passed as request params — passing them breaks the signature.
export function buildStartOrderUrl(opts: {
  shopId: string;
  key: string;
  priceEur: string; // "5.00"
  description: string; // ASCII only
  orderId: string;
  userId: string;
}): string {
