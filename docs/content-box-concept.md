...
- **Still deferred / needs owner:** creator Drop UI with **live signed-URL upload** (needs the private Supabase buckets), and real providers — CSAM, AI moderation, KYC (Veriff/Onfido), Paxum payouts, Twilio SMS (OTP + invitations). Design notes: `profiles.role`/`public_code` stay the signup default; box capabilities come from `box_members.role`. Refund/chargeback HOLD window before earnings become available is a documented future refinement (MVP: available immediately).
