...
- **Deferred to next phases:** upload + T&S (roadmap Phase 2), wallet/tokens (3), rentals (4), payouts (5), moderation console (6). Design note: `profiles.role`/`public_code` stay the signup default; box capabilities come from `box_members.role` (contextual). SMS provider (Twilio) still to be wired for invitations + OTP.
