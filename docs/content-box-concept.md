...
- **Phase 6 — Creator payouts ✅** (`0007_payouts.sql` + app): `payout_requests`; `request_payout()` locks the whole available creator balance into a payout at the **€50 (5000-token) threshold** (debits wallet immediately, idempotent); `decide_payout()` — admin **pay** (Paxum, logged) marks paid + provider ref, or **reject** refunds the wallet. `/earnings` page (available/pending/lifetime + request button, disabled <€50 + payout history); admin dashboard gains a **Payout requests** panel (Mark paid / Reject). Validated vs Postgres (threshold block, fund-lock, idempotency, reject-refund, no-re-decide, pay, RLS, exec-guard) + typecheck + build.
- **Phase 7 — Reports + moderation console ✅** (`0008_reports.sql` + app): `reports` (content/profile/box, 7 reasons, `urgent` flag for csam/nonconsensual/illegal, `due_at` = +7 days for the card-network takedown deadline). `/report` filing page; `POST /api/reports`; `POST /api/admin/reports/decision` (**takedown** suspends the reported content + writes a moderation_event, or **dismiss**). Admin dashboard now shows a **Reports** triage panel (urgent-first, OVERDUE flag) alongside Moderation queue, KYC, Payouts — completing the console. Validated vs Postgres (due-date, urgent, takedown→suspended + report actioned + mod event, RLS, no-auth-insert, reason check) + typecheck + build.
- **MVP roadmap COMPLETE (phases 0–7).** Remaining before launch: **Phase hardening** (rate limiting, monitoring, backups/DR, security review, OSS/VAT + DAC7 export) and the live provisioning + real providers below.
- **Creator Drop UI ✅** (`/drop` + `app/api/content/[id]/upload-url`): KYC gate (consent checkbox → `/api/kyc/submit`; blocks upload until verified), box selector (creator's active boxes), multi-file picker (image+video), title/desc/price → creates draft → **direct signed-URL upload to the private `content-originals` bucket** (`uploadToSignedUrl`) → finalize (runs the T&S pipeline). Typecheck + build verified; **live upload runs once the private buckets exist** (route returns a graceful "storage not configured" until then).
- **Buyer feed/discover + cart ✅** (`/boxes`, `/box/[code]` + `BoxFeed`): `/boxes` lists the user's boxes; `/box/[code]` is the membership-gated blurred feed (approved content across creators, one central feed). RLS-correct: membership verified via the user session, then blur previews signed with the service role (buyers can't read `content_files` directly). Cards show blurred preview (signed blur file, or gradient fallback), CNT code, creator code, photos/videos/24h, gold price; per-card **Rent 24h** + multi-select **cart** (slide-up bar, one call to `/api/rentals`) → redirects to My Rentals; already-rented items show "View". Insufficient balance → wallet. Matches the prototype design system. Typecheck + build verified; live blur thumbnails need the private buckets. (UI prevents re-renting an active item; backend still allows a fresh rental — a dedup guard can be added later.)
- **Hardening pass ✅** (`0009_rate_limits.sql` + app): DB-backed fixed-window **rate limiting** (`check_rate_limit`, works across serverless instances, fails open) applied to invitations (40/h), reports (20/h), rentals (120/h), KYC submit (10/h), Verotel charge (20/h). **Vercel Cron** (`vercel.json`) runs `/api/cron/expire` hourly; the route now also accepts Vercel's `Authorization: Bearer $CRON_SECRET`. Home page (`/`) is now a **nav hub** linking Boxes/Wallet/My Rentals/Drop/Earnings/Admin. Rate-limit validated vs Postgres; typecheck + build green.
- **Still deferred / needs owner:** real providers — CSAM, AI moderation, KYC (Veriff/Onfido), Paxum payouts, Twilio SMS (OTP + invitations); plus monitoring/backups/DR and live provisioning. **Step-by-step go-live instructions: `docs/content-box-runbook.md`.** Design notes: `profiles.role`/`public_code` stay the signup default; box capabilities come from `box_members.role`. Refund/chargeback HOLD window before earnings become available is a documented future refinement (MVP: available immediately).
