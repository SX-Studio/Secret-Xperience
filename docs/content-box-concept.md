...
- **Deferred to next phase:** full moderation console + reports (roadmap Phase 6 — reports table + triage, richer console, 7-day takedown), plus the creator Drop UI with **live signed-URL upload** (needs private Supabase buckets) and real CSAM/AI/KYC/Paxum providers. Design notes: `profiles.role`/`public_code` stay the signup default; box capabilities come from `box_members.role`. Refund/chargeback HOLD window before earnings become available is a documented future refinement (MVP: available immediately). SMS provider (Twilio) still to wire for OTP + invitations.
